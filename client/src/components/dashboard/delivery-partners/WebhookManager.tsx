...
// Define delivery-specific webhook events
const DELIVERY_WEBHOOK_EVENTS = [
  ...
  { id: "delivery.inProgress", label: "Delivery In Progress" },
  { id: "delivery.completed", label: "Delivery Completed" },
  { id: "delivery.cancelled", label: "Delivery Cancelled" },
...
export function WebhookManager({ type = "delivery" }: WebhookManagerProps) {
  ...
  const { data: allWebhooks, isLoading: isLoadingWebhooks } = useQuery({
  ...
  const webhooks = allWebhooks?.filter((webhook: Webhook) => 
    webhook.integrationType === type
  ) || [];
  
  // Fetch integrations to link webhooks to
  const { data: allIntegrations, isLoading: isLoadingIntegrations } = useQuery({
  ...
  const integrations = allIntegrations?.filter((integration: any) => 
      ...
      const response = await apiRequest(
        ...
        `/api/webhooks/${webhookId}/test`,
        {
          ...
          testPayload: {
            ...
            pickup: {
              address: "123 Restaurant Ave, City",
  ...
  const getIntegrationName = (id: number) => {
    ...
    const integration = allIntegrations.find((i: any) => i.id === id);
      ...
      <Tabs defaultValue="webhooks" className="w-full">
        ...
        <TabsContent value="webhooks" className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold">Delivery Webhooks</h2>
          ...
          {isLoadingWebhooks ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                You haven't created any delivery webhooks yet. Add your first webhook to receive real-time delivery updates.
              </AlertDescription>
            </Alert>
          ) : (
        ...
        <TabsContent value="add" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Set up webhooks to receive real-time notifications for delivery events
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Delivery Status Updates" {...field} />
                          </FormControl>
                          <FormDescription>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            The URL where delivery update events will be sent
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={() => (
                        <FormItem>
                          <div className="mb-4">
                            ...
                            <FormDescription>
                              Select which delivery events will trigger this webhook
                            </FormDescription>
                          </div>
                          {DELIVERY_WEBHOOK_EVENTS.map((event) => (
                            <FormField
                              key={event.id}
                              control={form.control}
                              name="events"
